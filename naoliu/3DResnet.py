...
[3, 8, 36, 3])
    ...
    def train(output="./logs"):
        #model = keras.models.load_model('./model/3Dresnet_32_3_18_100epoches_cc.model')
        K.set_image_dim_ordering('tf')
        x_train = np.load('./train_512_fenlei123_shuffle.npy')
        y_train = np.load('./label_512_fenlei123_shuffle.npy')
        x_train = x_train.reshape((2080, 23, 512, 512, 1))
        x_train_0 = np.zeros((1040, 32, 512, 512, 1))
        y_train_0 = np.zeros((1040, 3))
        for m in range(2080):
            if m%2==0:
                x_train_0[m//2,0:23,:,:,:]= x_train[m,:,:,:,:]
                y_train_0[m//2,:] = y_train[m,:]
        #y_train = y_train[0:1040,:]
        #y_train = np.zeros((200, 3))
        print(x_train_0.shape, y_train_0.shape)
        x_train, x_test, y_train, y_test = train_test_split(x_train_0, y_train_0, test_size=0.2, random_state=8)
        model = Resnet3DBuilder.build_resnet_18((32, 512, 512, 1), 3)
        model.compile(loss='categorical_crossentropy', optimizer='sgd', metrics=['accuracy'])
        model_checkpoint = ModelCheckpoint('./model/3Dresnet_auto.h5', monitor='auto',save_best_only=True,mode= 'max')
        model.fit(x_train, y_train, batch_size=2, class_weight ='auto',epochs=50,validation_data=(x_test, y_test), shuffle=True)
        #model.fit(x_train, y_train, batch_size=8,epochs=50,validation_data=(x_test, y_test), shuffle=True)
        model.save("./model/3Dresnet_32_3_18_50epoches_cc_shuffle.model")
        print("Optimization Finished!")

if __name__ == '__main__':
     Resnet3DBuilder.train()
